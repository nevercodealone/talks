...
async function getConfig() {
  ...
  const userConfig = {
    ...
    GOOGLE_FONTS: [{ family: "Source Sans Pro" }, { family: "Passion One" }],
  ...
  const FONTS_DONWLOAD = userConfig.GOOGLE_FONTS_FORMATS.map((format) =>
    fs.existsSync(`./dist/lib/css/${format}.css`)
  ).reduce((acc, bool) => acc && bool, true)
    ? false
    : true;

  const htmlList = await gchelpers.getEntries("./src/");
  const [entries, htmlPluginList] = gchelpers.getEntriesAndHTMLPlugins(
    htmlList,
    userConfig.FONTAWESOME_BACKEND == "svg"
  );
  ...
  return {
    ...
    plugins: [
      ...
      new CopyWebpackPlugin([
        ...
        { context: "src/content", from: "**/*", to: "content/" },
      ]),
      ...
      new webpack.DefinePlugin({
        HIGHLIGHT_LANGUAGES: JSON.stringify(
          Object.assign({}, userConfig.HIGHLIGHT_LANGUAGES)
        ),
        FA_CSS_LOCAL:
          userConfig.FONTAWESOME_BACKEND == "css" &&
          userConfig.FONTAWESOME_USE_LOCAL &&
          !isEnv("production-web-css"),
      }),
      ...
      /* Include fonts */
      new HtmlWebpackIncludeAssetsPlugin({
        assets: userConfig.GOOGLE_FONTS_FORMATS.map(
          (format) => `lib/css/${format}.css`
        ),
        append: true,
      }),
        ...
        apply: (compiler) => {
          compiler.hooks.afterEmit.tap("AfterEmitPlugin", (compilation) => {
            if (process.env.NODE_ENV != "dev-server") {
              exec(
                "rm -R ./src/_scripts/_generatedEntries",
                (err, stdout, stderr) => {
